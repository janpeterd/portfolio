...
{#if logo}
  <a
    {href}
    class="text-bold text-red flex-auto p-1 text-center text-xl hover:text-secondary {classes}">
    {@render children?.()}
  </a>
{:else}
  <a
    {href}
    class="group flex-auto p-1 text-center text-xl font-bold transition-all duration-300 ease-in-out hover:text-secondary {classes}">
    <span
